...
class Ion_auth_model extends CI_Model
{
    ...
    {
        $this->trigger_events('cnic_check');
        ...
        return $this->db->where('cnic', $cnic)
            ->group_by("id")
            ->order_by("id", "ASC")
            ->limit(1)
            ->count_all_results($this->tables['users']) > 0;
     ...
     * @author Mathew
     * */
    public function set_recruiter_group($id)
    {
        $data = array(
            'group_id' => 3,
            'user_id' => $id,
    ...
    public function register($username, $password, $email, $additional_data = array(), $groups = array())
    {
        ...
        $otp_activation = $this->config->item('otp_activation', 'ion_auth');
        ...
        $data = array(
            ...
            'created_on' => time(),
            'active' => ($otp_activation === false ? 1 : 0)
        );
    ...
    public function register_social($username, $password, $email, $additional_data = array(), $groups = array())
    {
        $this->trigger_events('pre_register');
    ...
    public function login($identity, $password, $remember = FALSE)
    {
        ...
        $this->trigger_events('extra_where');
        $check_column = $this->email_valid($identity) ? $this->identity_column : 'username';
        $query = $this->db->select($this->identity_column . ', username, email, users.id, password, active, last_login,user_type_id,is_block,first_name,last_name,salt,users_groups.group_id')
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($password === TRUE) {
                ...
                if ($user->is_block == 1) {
                    $this->trigger_events('post_login_unsuccessful');
                    $this->set_error('login_unsuccessful_is_block');

                    return FALSE;
                }
                if ($user->group_id != 2) {
                    $this->trigger_events('post_login_unsuccessful');
                    $this->set_error('login_unsuccessful_user_not_allowed');
    ...
    public function login_with_salt($salt)
    {

        $this->trigger_events('pre_login');

        $this->trigger_events('extra_where');
        $query = $this->db->select('username, email, id, password, active, last_login,user_type_id,is_block,first_name,last_name,salt')
            ->where('salt', $salt)
            ->limit(1)
            ->order_by('id', 'desc')
            ->get($this->tables['users']);
     ...
     * @author Ben Edmunds
     * */
    public function set_employee_group($id)
    {
        $data = array(
            'group_id' => 2,
            'user_id' => $id,
     ...
     * @author Ben Edmunds
     * */
    public function update_cmp_id($cmp_id, $user_id)
    {
        $data = array(
            'cmp_id' => $cmp_id
        );
    ...
    public function login_remembered_user()
    {
        ...
        //get the user
        $this->trigger_events('extra_where');
        $query = $this->db->select($this->identity_column . ', id, username, first_name, last_name, user_type_id, email, last_login')
    ...
    {
        $this->db->select('users.id as user_id,users.first_name as first_name,users.last_name as last_name,hb_countries.country_name as location,users.user_image_url as profile_image');
        $this->db->from('users');
        $this->db->join("hb_countries", "users.country = hb_countries.country_id", "LEFT");
    ...
    {
        $this->db->select('users.id as user_id,users.first_name as first_name,users.last_name as last_name,users.email,users.phone,users.gender,users.dob,user_setting.promo_setting,user_setting.update_by_sms_setting as sms_setting');
        $this->db->from('users');
        $this->db->join("user_setting", "users.id = user_setting.user_id", "INNER");
        $this->db->where('users.id', $id);
        $query = $this->db->get();
    ...
    public function get_user_contact_details($user_id,$address_id)
    {
        $this->db->select("users.first_name,users.last_name,users.email,users.phone,
        CONCAT('Block No: ',block,IF(judda!='', CONCAT(', Avenue: ',judda), ''),', Street No: ',street,
		...
		IF(extra_direction!='', CONCAT(', Extra Directions: ',extra_direction), ''), IF(area!='', CONCAT(', <br>Area: ',cities.city_name), '')
		) AS address",false);
        $this->db->from('users');
        $this->db->join("user_addresses", "users.id = user_addresses.user_id AND user_addresses.id = '$address_id'", "left");
    ...
    public function change_password_new($new, $email)
    {
        $this->db->select('*');
        $this->db->from('users');
