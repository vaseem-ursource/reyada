...
class Categories extends MY_Controller {
    ...
    public function Add()
		...
		$data['title'] = 'Categories';
		$data['ParentCategories'] = $this->Categories_model->get_all_categories();
		$data['ParentCategories'] = $this->Categories_model->get_all_parent_categories(0);
		$stmt='<select class="select2_group form-control" id="category" name="category" required><option value="" required>-- Choose --</option>
		...
		public function loop($data,$stmt,$space){
			$space.='&nbsp&nbsp&nbsp&nbsp';
					...
					$data['SubCategories']=$this->Categories_model->get_all_parent_categories($sub->cat_id);
					if($data['SubCategories']->num_rows())
					{
						$stmt=$this->loop($data,$stmt,$space);
					}else{
